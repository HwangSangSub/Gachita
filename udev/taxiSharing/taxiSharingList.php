...
// 투게더 닉네임
$mnSql = "  , ( SELECT mem_NickNm FROM TB_MEMBERS WHERE TB_MEMBERS.idx = A.taxi_MemIdx AND TB_MEMBERS.b_Disply = 'N' limit 1 ) AS memNickNm  ";
$mnSql2 = "  , ( SELECT mem_NickNm FROM TB_MEMBERS WHERE TB_MEMBERS.idx = A.taxi_MemIdx AND TB_MEMBERS.b_Disply = 'Y' limit 1 ) AS memNickNm2  "; //탈퇴회원
$query = "";
$query = "SELECT A.idx, A.taxi_MemIdx, A.taxi_MemId, A.taxi_Saddr, A.taxi_Eaddr, A.taxi_SDate, A.taxi_TPrice, A.taxi_Price, A.taxi_Per, A.taxi_State " ;
...
if ($taxiMemId != "" ) {  //고유회원 아이디
    $stmt->bindValue(":taxi_MemId",$taxiMemId);
}
...
$qstr = "fr_date=".urlencode($fr_date)."&amp;to_date=".urlencode($to_date)."&amp;findType=".urlencode($findType)."&amp;findword=".urlencode($findword)."&amp;od_status=".urlencode($od_status);
...
<div id="wrapper">
    <div id="container" class="">
		...
		<h1 id="container_title">매칭 관리</h1>
        ...
        <div>
        	...
        	<select name="findType" id="findType">
        		...
        		<option value="mem_Idx" <?if($findType=="mem_Idx"){?>selected<?}?>>회원고유번호</option>
...
-->
        ...
        <div class="tbl_head01 tbl_wrap">
			...
			-->
                ...
                <th scope="col" id="mb_list_mailc">투게더</th>  
   ...
   <?
	...
	-->
        ...
        <td headers="mb_list_id"><a href="taxiSharingSList.php?taxiSIdx=<?=$row['idx']?>" class="btn btn_05">투게더</a></td>
        <td headers="mb_list_id"><a href="#" class="btn btn_a<?=$taxi_State?>"><?=$taxiState?></a></td>
		<td headers="mb_list_mng" class="td_mng td_mng_s">
		  <a href="taxiSharingReg.php?mode=mod&idx=<?=$row['idx']?>&<?=$qstr?>&page=<?=$page?>" class="btn btn_03">상세</a>
