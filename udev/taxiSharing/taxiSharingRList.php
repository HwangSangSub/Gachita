...
$sql_search = " WHERE 1 = 1 AND A.taxi_state = 9 ";
if ($taxi_MChk != "" ) {
    $sql_search .= " AND C.taxi_MChk = :taxi_MChk ";
}else{
    $sql_search .= " AND C.taxi_MChk IN ('1', '2', '3', '4')";
...
if ($taxi_MChk != "" ) {
    $cntStmt->bindValue(":taxi_MChk",$taxi_MChk);
}
...
$query = "";
$query = "SELECT A.idx, B.idx as c_idx, A.taxi_MState, A.reg_CDate, C.taxi_MemId, C.taxi_RMemId, C.taxi_MChk, B.taxi_MType, B.taxi_OMType, B.taxi_CPart, B.taxi_CRPart, B.taxi_CMemo, A.reg_Date, D.taxi_OrdNo, D.taxi_OrdPrice, D.reg_Date as order_Date {$mnSql} {$mnSql2} {$mnSql3} {$mnSql4}  " ;
...
$query .= " LEFT OUTER JOIN TB_RTAXISHARING_INFO E ON C.idx = E.taxi_RIdx ";
...
if ($taxi_MChk != "" ) {
    $stmt->bindValue(":taxi_MChk",$taxi_MChk);
}
...
//완료변수설정 값 조회	
$chkQuery = "";
$chkQuery .= "SELECT conComp1_Max_D, conComp1_H, conComp2_Min_D, conComp2_Max_D, conComp2_H, conComp3_Min_D, conComp3_Max_D, conComp3_H, conComp4_Min_D, conComp4_H from TB_CONFIG ";
$chkStmt = $DB_con->prepare($chkQuery);
$chkStmt->execute();
while($chkrow = $chkStmt->fetch(PDO::FETCH_ASSOC)) {
	$conComp1Max_D = $chkrow['conComp1_Max_D'];		//조건 1 : 최대거리
	$conComp1H = $chkrow['conComp1_H'];				//조건 1 : 제한시간
	$conComp2Min_D = $chkrow['conComp2_Min_D'];		//조건 2 : 최소거리
	$conComp2Max_D = $chkrow['conComp2_Max_D'];		//조건 2 : 최대거리
	$conComp2H = $chkrow['conComp2_H'];				//조건 2 : 제한시간
	$conComp3Min_D = $chkrow['conComp3_Min_D'];		//조건 3 : 최소거리
	$conComp3Max_D = $chkrow['conComp3_Max_D'];		//조건 3 : 최대거리
	$conComp3H = $chkrow['conComp3_H'];				//조건 3 : 제한시간
	$conComp4Min_D = $chkrow['conComp4_Min_D'];		//조건 4 : 최소거리
	$conComp4H = $chkrow['conComp4_H'];				//조건 4 : 제한시간

	$concomp1 = "이동거리가 ".$conComp1Max_D."KM 미만 구간이지만, 이동시간 ".$conComp1H."시간 초과 한 경우";
	$concomp2 = "이동거리가 ".$conComp2Min_D."KM 이상".$conComp2Max_D."KM 미만 구간이지만, 이동시간 ".$conComp2H."시간 초과 한 경우";
	$concomp3 = "이동거리가 ".$conComp3Min_D."KM 이상".$conComp3Max_D."KM 미만 구간이지만, 이동시간 ".$conComp3H."시간 초과 한 경우";
	$concomp4 = "이동거리가 ".$conComp4Min_D."KM 이상 구간이지만, 이동시간 ".$conComp4H."시간 초과 한 경우";
}
$DB_con = null;
...
<div id="wrapper">
    <div id="container" class="">
        ...
        <h1 id="container_title">완료처리 관리</h1>
        ...
        <div>
            <strong>취소조건</strong>
        	<span class="bg <? if ($taxi_MChk == "") { ?>all_on<? } ?>">
            <input type="radio" name="taxi_MChk" value="" id="taxi_MChk_all" <?php echo get_checked($taxi_MChk, ''); ?>>
            <label for="taxi_MChk_all">전체</label>
        	</span>
        	<span class="bg <? if ($taxi_MChk == "1") { ?>c01_on<? } ?>">
            <input type="radio" name="taxi_MChk" value="1" id="taxi_MChk_1" <?php echo get_checked($taxi_MChk, '1'); ?>>
            <label for="taxi_MChk_1">조건1</label>
        	</span>
        	<span class="bg <? if ($taxi_MChk == "2") { ?>c02_on<? } ?>">
            <input type="radio" name="taxi_MChk" value="2" id="taxi_MChk_2" <?php echo get_checked($taxi_MChk, '2'); ?>>
            <label for="taxi_MChk_2">조건2</label>
        	</span>
        	<span class="bg <? if ($taxi_MChk == "3") { ?>c03_on<? } ?>">
            <input type="radio" name="taxi_MChk" value="3" id="taxi_MChk_3" <?php echo get_checked($taxi_MChk, '3'); ?>>
            <label for="taxi_MChk_3">조건3</label>
        	</span> 
        	<span class="bg <? if ($taxi_MChk == "4") { ?>c04_on<? } ?>">
            <input type="radio" name="taxi_MChk" value="4" id="taxi_MChk_4" <?php echo get_checked($taxi_MChk, '4'); ?>>
            <label for="taxi_MChk_4">조건4</label>
        	</span> 
        </div>
        ...
        <div class="tbl_head01 tbl_wrap">
            ...
            <tr>
                ...
                <th scope="col" id="mb_list_mailc">취소조건</th>  
                <th scope="col" id="mb_list_mailc">결제주문번호</th> 
                <th scope="col" id="mb_list_mailc">결제금액</th>   
                <th scope="col" id="mb_list_mailc">결제일(=이동중등록일)</th>  
                <th scope="col" id="mb_list_mailc">노선생성일</th>
   ...
   <?
    ...
    <tr class="<?=$bg?>">
        ...
        <td headers="mb_list_id"><a href="/udev/config/configReg.php"/><span title="<?=$taxiMChkT?>"><?=$taxiMChk?></span></a></td>
...
<? if($_COOKIE['du_udev']['id'] != 'admin2'){ ?>
		  <a href="javascript:;" onclick="taxi_Cancle('<?=$row['idx']?>','Y');" class="btn btn_01">완료</a>
		  <a href="javascript:;" onclick="taxi_Cancle('<?=$row['idx']?>','N');" class="btn btn_02">취소</a>
<? } ?>
		</td>
    </tr>
    <? 
