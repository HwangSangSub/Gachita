...
if ($type == 'Y') { //
    ...
    if ($taxi_PDisply == 'N') {
        ...
        //취소사유메모 기록
        $upMQquery33 = "UPDATE TB_SMATCH_STATE SET taxi_CMemo = '본사확인 후 완료처리', taxi_Disply ='Y', taxi_PDisply = :taxi_PDisply WHERE taxi_SIdx = :taxi_SIdx LIMIT 1";
 ...
 투게더(' . $memRNickNm . ') 님이 메이커(' . $memNickNm . ')님에게 요청 포인트 총 ' . number_format($taxi_OrdPoint) . '원에서 수수료 ' . $totalDc . '%를 차감한 요청 포인트 ' . number_format($taxiOrdPoint) . '원에서 수수료 ' . $totalDc . '%인 수익 ' . number_format($taxiPoint) . '원를 적립' . "";
    ...
    $chkState = "10";  //거래완료
    if ($taxi_PDisply == 'Y') {
        $nmsg = "본사확인 후 완료처리되었습니다. 포인트가 양도되었습니다.";
    } else if ($taxi_PDisply == 'N') {
        $nmsg = "본사확인 후 완료처리되었습니다. 포인트는 양도대기 중 입니다.";
    }
    $ntitle = "";
    $nmsg = $nmsg;
    ...
    $rchkState = "10";  //거래완료
    if ($taxi_PDisply == 'Y') {
        $rmsg = "본사확인 후 완료처리되었습니다. 포인트가 양도되었습니다.";
    } else if ($taxi_PDisply == 'N') {
        $rmsg = "본사확인 후 완료처리되었습니다. 포인트는 양도대기 중 입니다.";
    }
    $rtitle = "";
    $rmsg = $rmsg;
