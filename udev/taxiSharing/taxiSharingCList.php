...
$sql_search = " WHERE 1 = 1 AND A.taxi_state = 8";
if ($taxi_MState != "") {
...
//전체 카운트
$cntQuery = "";
$cntQuery = "SELECT COUNT(A.idx) AS cntRow FROM TB_STAXISHARING A LEFT OUTER JOIN TB_SMATCH_STATE B ON A.Idx = B.taxi_SIdx {$sql_search}  ";
//echo $cntQuery."<BR>";
//exit;
...
$query = "";
$query = "SELECT A.idx, B.idx AS cancle_Idx, B.taxi_SIdx, C.taxi_MemId, C.taxi_RMemId, B.taxi_MType, B.taxi_OMType, B.taxi_CanRChk, B.taxi_CPart, B.taxi_CRPart, B.taxi_CMemo, B.reg_CRDate, A.reg_CDate, A.taxi_MState {$mnSql} {$mnSql2} {$mnSql3} {$mnSql4}  ";
...
<div id="wrapper">
	<div id="container" class="">
		<div class="container_wr">
			<h1 id="container_title">취소 내역</h1>
			...
			<div class="local_desc01 local_desc">
				<p>
					매칭중, 매칭요청, 예약요청, 예약요청완료(예약완료 30분전)의 경우는 리스트에서 제외됬습니다. <br>
				</p>
			</div>
			<nav class="pg_wrap">
						...
						<tbody>
							...
							if ($numCnt > 0) {
								...
								while ($row = $stmt->fetch()) {
									...
									$regCDate =  $row['reg_CDate'];

									$taxi_MType = $row['taxi_MType'];

									if ($taxi_MType == "c") {
										$taxiMType =  $memNickNm_1 . "<Br>(투게더)";
									} else if ($taxi_MType == "p") {
										$taxiMType = $memNickNm . "<br>(메이커)";
									}
		...
		-->
										...
										<td headers="mb_list_id"><?= $taxiCanRChkNm ?></td>
										<td headers="mb_list_id"><?= $taxiCRPart ?></td>
										<td headers="mb_list_id"><?= $taxiCMemo ?></td>
										<td headers="mb_list_id"><?= ($regCRDate == '' ? $regCDate : $regCRDate) ?></td>
										<td headers="mb_list_mng" class="td_mng td_mng_s">
											<? if ($cancle_Idx == "") { ?>
												-
											<? } else { ?>
												<a href="taxiSharingCReg.php?mode=mod&idx=<?= $cancle_Idx ?>&<?= $qstr ?>&page=<?= $page ?>" class="btn btn_03">상세</a>
											<? } ?>
										</td>
									</tr>
								<?
