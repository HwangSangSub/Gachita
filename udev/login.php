<?
	...
	if( $du_udev['id']!= "" ) {
		...
		if ( $du_udev['lv']== 0 ||  $du_udev['lv'] == 1 ||  $du_udev['lv'] == 2   ) { //최고권한관리자 	
			//header("location:/udev/member/memberList.php");
			header("location:/udev/main/main.php");
			exit;
		} else {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if($.trim(response) == 'success') {
						//$('#loginform').attr('action','/udev/member/memberList.php').submit();
						$('#loginform').attr('action','/udev/main/main.php').submit();
