...
//목록
$query = "";
$query = "SELECT idx, memLv, memLv_Name, memLv_Nick, memLv_MatName, memIconFile, memIconInfoFile, memLv_Color, memMatCnt, memDc FROM TB_MEMBER_LEVEL {$sql_search} {$sql_order} limit  {$from_record}, {$rows}";
		...
		<div class="container_wr">
			...
			<form name="fmlist" id="fmlist" method="post" autocomplete="off">
						...
						<tbody>
							...
							if ($numCnt > 0) {
								...
								while ($row = $stmt->fetch()) {
									$idx = $row['idx'];
									$memLv_MatName = $row['memLv_MatName'];
									$memLv_Name = $row['memLv_Name'];
									$memLv = $row['memLv'];
									$memDc = $row['memDc'];
							...
							?>
									<tr class="<?= $bg ?>">
										<td headers="mb_list_id"><?= $memLv_Name ?></td>
										<td headers="mb_list_id"><?= $memLv ?></td>
										<td headers="mb_list_id"><?= $memLv_MatName ?></td>
										<td headers="mb_list_id"><?= $mDc ?></td>
										<td headers="mb_list_mng" class="td_mng td_mng_s">
											<? if ($_COOKIE['du_udev']['id'] != 'admin2') { ?>
												<a href="memManagerReg.php?mode=mod&idx=<?= $idx ?>&<?= $qstr ?>&page=<?= $page ?>" class="btn btn_03">수정</a>
				...
				<div class="btn_fixed_top">
					<? if ($_COOKIE['du_udev']['id'] != 'admin2') { ?>
						<a href="memManagerReg.php" id="memLv_add" class="btn btn_01">회원등급 추가</a>
					<? } ?>
				</div>
