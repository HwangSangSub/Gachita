...
if ($mode == "reg") {
	...
	//회원 기본 수정
	$upQquery = "UPDATE TB_MEMBERS SET mem_NickNm = :mem_NickNm, mem_Pwd = :mem_Pwd,  mem_Lv = :mem_Lv, mem_Tel = :mem_Tel, mem_Birth = :mem_Birth, mem_ImgFile = :mem_ImgFile WHERE mem_Id = :mem_Id LIMIT 1";
	...
	foreach ($array as $k => $v) {
		...
		if ($num == "0" && $cntNum == "0") { // 매칭신청 진행중인 회원이 없을 경우는 탈퇴처리
			...
			$upQquery = "UPDATE TB_MEMBERS SET mem_Tel = :mem_Tel, mem_Birth = :mem_Birth, mem_Sex = :mem_Sex, mem_ImgFile = :mem_ImgFile, mem_Haddr = :mem_Haddr, mem_Oaddr = :mem_Oaddr, mem_Point = :mem_Point, mem_MatCnt = :mem_MatCnt, b_Disply = :b_Disply, leaved_Date = :leaved_Date  WHERE idx = :idx LIMIT 1";
			$upStmt = $DB_con->prepare($upQquery);
			$upStmt->bindparam(":mem_Tel", $mem_Tel);
			$upStmt->bindparam(":mem_Birth", $mem_Birth);
