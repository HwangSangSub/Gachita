...
if ($mem_ChCode != "") {
	$sql_search .= " AND etc.mem_ChCode = :mem_ChCode";
}
...
//전체 카운트
$cntQuery = "";
$cntQuery = "SELECT COUNT(A.mem_Id) AS cntRow FROM TB_MEMBERS A INNER JOIN TB_MEMBERS_ETC etc ON A.idx = etc.mem_Idx LEFT OUTER JOIN TB_MEMBERS_INFO B ON B.mem_Idx = A.idx INNER JOIN TB_MEMWITHDRAWL C ON A.idx = C.mem_Idx  {$sql_search} ";
// echo $cntQuery."<BR>";
...
//목록
$query = "";
$query = "  SELECT A.idx, A.mem_Id, A.mem_NickNm, A.mem_OS, A.reg_Date,B.mem_Secede, B.mem_SecedeEtc, B.mem_SnsChk, B.leaved_Date, C.mem_Tel, C.mem_Point  FROM TB_MEMBERS A ";
$query .= " INNER JOIN TB_MEMBERS_ETC etc ON etc.mem_Idx = A.idx ";
$query .= " LEFT OUTER JOIN TB_MEMBERS_INFO B ON B.mem_Idx = A.idx ";
$query .= " INNER JOIN TB_MEMWITHDRAWL C ON A.idx = C.mem_Idx ";
					...
					<table>
						...
						<thead>
							...
							<tr>
								...
								<th scope="col" id="mb_list_mailr">잔여포인트</th>
						...
						<tbody>
							...
							?>
									...
									<tr class="<?= $bg ?>">
										...
										<td headers="mb_list_id" class="td_id_200"><?= $row['mem_Id'] . "<BR>(" . $row['mem_NickNm'] . ")" ?></td>
										<td headers="mb_list_id"><?= $row['mem_Tel'] ?></td>
										<td headers="mb_list_id"><?= number_format($row['mem_Point']) ?></td>
