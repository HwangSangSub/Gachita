<?
	...
	//회원레벨
	$query = "SELECT * FROM TB_MEMBER_LEVEL WHERE memLv > 3 ORDER BY memLv DESC";
...
<div id="wrapper">
    ...
    <div id="container" class="">
		...
		<input type="hidden" name="taxi_PState" id="taxi_PState" value="9"><!-- 관리자에 의한 적립/차감 -->
			...
			<tr height="45">
				...
				<td class="objects">
					...
					<div id="mType2" style="display:none;width:500px;">
						<input type="text" class="frm_input" name="taxi_MemId" id="taxi_MemId" style="width:60%;" placeholder="포인트 적립(차감) 회원아이디" readonly onclick="popup('pointIdSearch.php', 600, 400)" value="<?= $id?>">&nbsp;<a href="javascript:popup('pointIdSearch.php', 600, 400)" class="btn btn_02">회원검색</a>
					</div>					
				</td>
			</tr>
			<tr height="45">
				<th align="center">적립(차감)포인트</th>
				<td class="objects">
					<input type="text" class="frm_input" name="taxi_OrdPoint" id="taxi_OrdPoint" style="width:95%;" placeholder="적립(차감) 포인트">
				</td>
			</tr>		
			<tr height="45">
				<th align="center">적립(차감)포인트 메모</th>
				<td class="objects">
					<input type="text" class="frm_input" name="taxi_Memo" id="taxi_Memo" style="width:95%; " placeholder="포인트적립(차감) 사유 ">
				</td>
			</tr>
			<tr height="45">
				<th align="center">포인트 적립구분</th>
				<td class="objects">
		...
		function fmember_submit(f)
		{
			if (!f.taxi_OrdPoint.value) {
				alert('포인트 적립(차감)할 포인트를 입력해주세요.');
				return false;
			}
			if (!f.taxi_Sign.value) {
				alert('포인트 적립구분을 선택해주세요.');
				return false;
			}
