<?
	...
	$cntQuery .= " ,SUM(CASE WHEN A.b_Disply = 'D' THEN 1 ELSE 0 END) AS D_CNT " ;
	...
	$cntQuery .= " ,SUM(CASE WHEN B.mem_Card = '1' THEN 1 ELSE 0 END) AS Card_CNT " ;
	$cntQuery .= " ,SUM(CASE WHEN E.mem_Sex = '0' THEN 1 ELSE 0 END) AS male_CNT " ;
	$cntQuery .= " ,SUM(CASE WHEN E.mem_Sex = '1' THEN 1 ELSE 0 END) AS female_CNT " ;
	$cntQuery .= " FROM TB_MEMBERS A ";
	$cntQuery .= " INNER JOIN TB_MEMBERS_ETC B ON A.idx = B.mem_Idx";
	$cntQuery .= " INNER JOIN TB_MEMBERS_INFO E ON A.idx = E.mem_Idx";
	$cntQuery .= " LEFT OUTER JOIN TB_PAYMENT_CARD C ON A.mem_Id = C.card_Mem_id";
	$cntQuery .= " LEFT OUTER JOIN TB_MEMWITHDRAWL D ON A.idx = D.mem_Idx AND D.mem_Lv NOT IN ('0')";
...
//exit;
	...
	while($row = $cntStmt->fetch()) {
		...
		$male_CNT = $row['male_CNT'];
		$female_CNT = $row['female_CNT'];
	}
	...
	$query .= " ,SUM(CASE WHEN A.b_Disply = 'D' THEN 1 ELSE 0 END) AS D_CNT " ;
	$query .= " ,SUM(CASE WHEN A.mem_Os = '0' THEN 1 ELSE 0 END) AS And_CNT " ;
	$query .= " ,SUM(CASE WHEN A.mem_Os = '1' THEN 1 ELSE 0 END) AS Ios_CNT " ;
	$query .= " ,SUM(CASE WHEN B.mem_Card = '1' THEN 1 ELSE 0 END) AS Card_CNT " ;
	$query .= " ,SUM(CASE WHEN E.mem_Sex = '0' THEN 1 ELSE 0 END) AS male_CNT " ;
	$query .= " ,SUM(CASE WHEN E.mem_Sex = '1' THEN 1 ELSE 0 END) AS female_CNT " ;
	$query .= " FROM TB_MEMBERS A ";
	$query .= " INNER JOIN TB_MEMBERS_ETC B ON A.idx = B.mem_Idx";
	$query .= " INNER JOIN TB_MEMBERS_INFO E ON A.idx = E.mem_Idx";
	$query .= " LEFT OUTER JOIN TB_PAYMENT_CARD C ON A.mem_Id = C.card_Mem_id";
	$query .= " LEFT OUTER JOIN TB_MEMWITHDRAWL D ON A.idx = D.mem_Idx AND D.mem_Lv NOT IN ('0')";
...
<div id="wrapper">
    <div id="container" class="">
		...
		<div class="local_ov01 local_ov">
			...
			<span class="btn_ov01"><span class="ov_txt">기간 내 휴먼회원수 </span><span class="ov_num"><?=number_format($D_CNT);?>명 </span>&nbsp;
			...
			<span class="btn_ov01"><span class="ov_txt">기간 내 카드등록수 </span><span class="ov_num"><?=number_format($Card_CNT);?>명 </span>&nbsp;
			<span class="btn_ov01"><span class="ov_txt">기간 내 남자유저수 </span><span class="ov_num"><?=number_format($male_CNT);?>명 </span>&nbsp;
			<span class="btn_ov01"><span class="ov_txt">기간 내 여자유저수 </span><span class="ov_num"><?=number_format($female_CNT);?>명 </span>&nbsp;
		</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
		...
		<th scope="col" id="mb_list_dcnt">휴먼자수</th>		
		<th scope="col" id="mb_list_andcnt">안드로이드</th>		 
		<th scope="col" id="mb_list_loscnt">아이폰</th>
		<th scope="col" id="mb_list_cardcnt">카드등록자</th>
		<th scope="col" id="mb_list_malecnt">남자</th>
		<th scope="col" id="mb_list_femalecnt">여자</th>
    </tr>
    </thead>
    <tbody>
	...
	if($numCnt > 0)   {
       ...
       // $bg = 'bg'.($stmt->fetch()%2);
		...
		$malecnt = $row['male_CNT'];
		$femalecnt = $row['female_CNT'];
    ?>
    ...
    <tr class="<?=$bg?>">
		...
		<td headers="mb_list_dcnt"><?=$dcnt?></td>
		<td headers="mb_list_andcnt"><?=$andcnt?></td>
		<td headers="mb_list_ioscnt"><?=$ioscnt?></td>
        <td headers="mb_list_cardcnt"><?=$cardcnt?></td>
        <td headers="mb_list_malecnt"><?=$malecnt?></td>
        <td headers="mb_list_femalecnt"><?=$femalecnt?></td>
    </tr>
    <? 
