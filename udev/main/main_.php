<?
	...
	//목록
	$query = "";
	$query = "SELECT A.idx, A.mem_Id, A.mem_NickNm, A.mem_Tel, A.reg_Date, A.mem_Os, A.b_Disply, A.mem_Code, B.mem_Point, B.mem_MatCnt, " ;
	$query .= "  B.mem_McCnt, B.mem_Card";
	$query .= " {$memLvSql} FROM TB_MEMBERS A ";
	$query .= " LEFT OUTER JOIN TB_MEMBERS_ETC B ON B.mem_Idx = A.idx  {$sql_search} {$sql_order} limit  {$from_record}, {$rows} ";
	//echo $query."<BR>";
	//exit;
