...
function setPhotoToEditor(oFileInfo) {
	for (var i = 0; i < oFileInfo.length; i++) {
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			// opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', ["<img src='./upload/" + oFileInfo[0].sFileURL + "' width='600' />"]);
			opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', ["<img alt='" + oFileInfo[i].sFileName + "' src='" + oFileInfo[i].sFileURL + "' /> <br>"]);

			//본문에 바로 tag를 넣는 방법 (oFileInfo는 String으로 <img src=....> )
			// opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', oFileInfo[i]);
		}
	}
}
